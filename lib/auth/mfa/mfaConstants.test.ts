...
test('honours developer vs organisation destinations and rejects open redirects', () => {
  ...
  assert.equal(safePostMfaPath('/dashboard/change-organisation', '/dashboard'), '/dashboard/change-organisation')
...
test('developer login next is not rewritten to the organisation dashboard', () => {
  ...
  assert.equal(mfaVerifyHref('/dashboard/change-organisation'), '/auth/mfa?next=%2Fdashboard%2Fchange-organisation')
