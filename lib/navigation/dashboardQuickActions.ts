...
export function getDashboardQuickActions(user: User, organization: Organization | null): DashboardQuickAction[] {
  return ALL_ACTIONS.filter((item) => {
    ...
    ...item,
    label: getNavigationLabel(organization?.settings, item.navigationLabelKey, item.label),
  }))
}
