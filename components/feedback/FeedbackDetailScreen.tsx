...
export function FeedbackDetailScreen({ ideaId }: { ideaId: string }) {
  ...
  return (
    <div className="space-y-4">
      <Link href="/dashboard/ideas" className="btn sm ghost">
        ← Ideas
      </Link>
      ...
      <div className="card pad">
        <div className="flex items-start gap-3">
          <button
            type="button"
            onClick={() => user && void toggleVote(suggestion, user.id)}
            className={`flex min-w-[64px] flex-col items-center rounded-xl px-2 py-3 text-lg font-extrabold ${
              hasVoted(votes, suggestion.id, user?.id)
                ? 'bg-[var(--task-t)] text-[var(--task)]'
                : 'bg-[var(--soft)] text-[var(--ink2)]'
            }`}
          >
            {suggestion.voteCount}
            <span className="text-[10px] font-semibold uppercase">Vote</span>
          </button>
      ...
      <section className="card pad">
        ...
        <div className="mt-3 space-y-3">
          {comments.length === 0 ? <p className="text-sm text-[var(--ink3)]">No comments yet.</p> : null}
