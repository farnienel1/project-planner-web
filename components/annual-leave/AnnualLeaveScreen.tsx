...
function RemoveBookingButton({
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg border border-red-200 bg-red-50 text-red-600 transition-colors hover:border-red-300 hover:bg-red-100 hover:text-red-700"
      aria-label={label}
      title={label}
    >
...
function BookingRow({
  ...
  const displayStatus =
    booking.cancellationRequestedAt != null ? 'Cancellation pending' : statusLabels[booking.status]
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0 flex-1">
          <div className="flex items-start justify-between gap-2">
            ...
            {onDelete && <RemoveBookingButton onClick={onDelete} label="Delete booking" />}
          </div>
          <p className="mt-0.5 text-xs text-slate-500">{booking.timeSlot}</p>
          <p className={`mt-0.5 text-xs font-semibold ${displayStatusColor}`}>
            {displayStatus}
          </p>
        </div>
              ...
              <div className="space-y-1">
                {selectedDates.map((d) => (
                  ...
                  >
                    ...
                    <RemoveBookingButton
                      label={`Remove ${fmtDate(d)}`}
                      onClick={() =>
                        setSelectedDates((prev) => prev.filter((entry) => !isSameDay(entry, d)))
                      }
                    />
                  </div>
                ))}
...
function OperativeView({
  ...
  const submitCancellation = async (booking: HolidayBooking) => {
    if (!organization?.id || !user) return
    if (
      !window.confirm(
        `Request cancellation of your approved leave on ${fmtDate(booking.startDate)}? Your line manager will need to approve.`
      )
    ) {
      return
    }
    ...
    try {
      ...
      setSuccess('Cancellation request sent to your line manager.')
      setTimeout(() => setSuccess(null), 4000)
    } finally {
      setSaving(false)
    }
          ...
          {myApproved.length === 0 ? (
            ...
            myApproved.map((b) => (
              <BookingRow
                key={b.id}
                booking={b}
                showCancel={!b.cancellationRequestedAt}
                onCancel={() => submitCancellation(b)}
              />
            ))
          )}
        </div>
